...
from app.utils.common import check_and_video_type
...
class ExternaVideo(View):
    ...
    def post(self,request):
        ...
        print(name,image,video_type,nationality,from_to,info)

        if not all([name,image,video_type,nationality,from_to,info]):
            return redirect('{}?error={}'.format(reverse('externa_video'),'缺少必须项'))
...
class VideoSubView(View):
    ...
    def post(self,request,video_id):
        ...
        return redirect(reverse('video_sub',kwargs={'video_id':video.id}))
