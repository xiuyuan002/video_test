...
def render_to_response(request,template,data=None):
    ...
    #esult['csrf_token']=('<input type="hidden" name="csrfmiddlewaretoken" value={0} />'.format(request.META['CSRF_COOKIE']))
    result['csrf_token']=('<input type="hidden" name="csrfmiddlewaretoken" value={0} />'.format(request.COOKIES['csrftoken']))
